...
public class DeterministicSpectra {
  ...
  public static final String PGACALC_VERSION = "1.3.335";
	private static final String LOG_PREFIX = UtilFns.getLogPrefix(DeterministicSpectra.class);
	private static double calcPga(Logger log, Map<Gmm, Double> gmmWtMap, double mag, double rJB, double rx,
			double rRup, double vs30, boolean vsInf) {
		Gmm gmm;
    ...
    if (args.length < 7) {
	  ...
	  boolean vsInf = Site.VS_INF_DEFAULT;
    if (args.length > 7) {
      try {
      ...
      if (args.length > 8 && !Boolean.parseBoolean(args[8])) {
        vsInf = false;
      }
    }
		...
		Map<Gmm, Double> gmmWtMap = GmmsParser.gmmWeightMap(region.toString(), distance, input);
		log.info(String.format("site=%s, region=%s, mag=%f, depth=%f, rJB=%f, rX=%f, rRup=%f, vs30=%f, vsInf=%s", siteName,
				region.toString(), mag, depth, rJB, rx, rRup, vs30, Boolean.toString(vsInf)));
		return calcPga(log, gmmWtMap, mag, rJB, rx, rRup, vs30, vsInf);
	}
	...
	public static void printUsage(Appendable out) {
	  try {
	    String cmdSuffix = DeterministicSpectra.class.getName()
	        + " \"site name\" siteLon siteLat eqMag eqLon eqLat eqDepth [vs30 vsInf]";
      ...
      out.append("),\n"
          + "and vsInf is the optional vsInf value (true if Vs30 was inferred or false if measured, default is ");
      out.append(Boolean.toString(Site.VS_INF_DEFAULT));
	    out.append(").");
	    out.append(
